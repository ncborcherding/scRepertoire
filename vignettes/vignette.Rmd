...
It is now easy to create the contig list from a multiplexed experiment by first generating a single-cell RNA object (either Seurat or Single Cell Experiment),  loading the filtered contig file and then using `createHTOContigList()`. This function will return a list, seperated by the groupBy variable(s)

```{r, eval = F}
contigs <- read.csv(".../outs/filtered_contig_annotations.csv")

contig.list <- createHTOContigList(contigs, Seurat.Obj, groupBy = "HTO_maxID")
```
...
```{r}
clonalDiversity(combined, cloneCall = "gene", groupBy = "sample", 
