...
#Making data frame to plot, include TCRA and TCRA cluster
plot.df <- data.frame(barcode = sub_combined$barcode, 
                      TCR1 = sub_combined$cdr3_nt1, 
                      cluster = sub_combined$TCRA_cluster)
plot.df <- na.omit(reshape2::melt(plot.df, id.vars = "barcode"))
...
#Removing TCR or clusters with values <= 2
plot.df <- plot.df[plot.df$n > 2,]

ggplot(plot.df, aes(x = variable, y = n)) + 
