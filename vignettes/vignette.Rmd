...
The **clonal.split** represents the ranking of clonotypes by copy or frequency of occurrence, meaning 1:10 are the top 10 clonotypes in each sample. The default bins are under the **clonal.split** variable in the function and can be adjusted, but they are as follows at baseline. 
...
We can also use Hill numbers to estimate the rarefaction using the the abundance of clones across groupings. This relies on the [iNEXT](https://cran.r-project.org/web/packages/iNEXT/index.html) with the accompanying [manuscript](http://chao.stat.nthu.edu.tw/wordpress/paper/120_pdf_appendix.pdf). 
