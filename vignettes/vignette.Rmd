...
+  "gene+nt" - use the VDJC genes comprising the TCR/Ig + the nucleotide sequence of the CDR3 region. This is the [proper definition of clonotype](https://www.ncbi.nlm.nih.gov/pubmed/19568742).

Important to note, that the clonotype is called using essentially the combination of VDJC genes or nt/aa CDR3 sequences for both loci. As of this implementation of scRepertoire, clonotype calling is not incorporating small variations within the CDR3 sequences. As such the *gene* approach will be the most sensitive, while the use of *nt* or *aa* moderately so, and the most specific for clonotypes being *gene+nt*. Additionally, the clonotype call is trying to incorporate both loci, *i.e*, both *TCRA* and *TCRB* chains and if a single cell barcode has multiple sequences identified (*i.e.*, 2 TCRA chains expressed in one cell). Using the 10x approach, there is a subset of barcodes that only return one of the immune receptor chains, the unreturned chain is assigned an *NA* value.
...
Lastly on the basic visualization side, we can look at the length distribution of the CDR3 sequences by calling the *lengthContig()* function. Importantly, unlike the other basic visualizations, the **cloneCall** can only be "nt" or "aa". Due to the method of calling clonotypes as outlined above, the length should reveal a multimodal curve, this is a product of using the **NA** for the unreturned chain sequence and multiple chains within a single barcode. 
...
A number of users requested a visualization from the work of [Wu, et al 2020, PMID: 32103181](https://pubmed.ncbi.nlm.nih.gov/32103181/) that allows for the direct comparison of clonotypes between 2 samples. ```scatterClonotype()``` will organize two samples from the combinedTCR/BCR product, count the relative clonotypes, and produce a scatter plot comparing the two samples. 
...
If performed over the number of samples, such as the list elements, **group** can used to calculate only the clusters on the setting of patient sample (group = "Patient") or tissue type (group = "Type"). This will add the selected group to the beginning of the cluster designation.
...
We can also look at the count of cells by cluster assigned into specific frequency ranges, by using the `occupiedscRepertoire()` function and selecting the **x.axis** to display cluster or other variables in the meta data of the single cell object. In addition
**proportion** can be used to looked at a relative level groupings by **x.axis**, label will still return absolute numbers. 

```{r}
occupiedscRepertoire(screp_example, x.axis = "cluster")
occupiedscRepertoire(screp_example, x.axis = "cluster", proportion = TRUE, label = FALSE)
```
...
There is a high degree of overlap in the feature space of expanded clonotypes, not only in the adaptive receptor genes that form the clonotype, but also in potentially functional genes. In order to identify divergent clusters in shared clonotypes, we can regress out the effects of the expanded clonotypes on the feature space using ```regressClonotype()```, by first removing the adaptive receptor genes from the variable gene list and then using the [Harmony R package](https://github.com/immunogenomics/harmony) to re-integrate/re-embed the single-cell data using the clonotype variable as a correcting factor. We can also add additional variables to regress using the **groupVariable** parameter, such as batch or orig.ident. **Note: If running a number of single-cells with a diverse clonotype repertoire, this function may take a lot of time**

**groupVariable**  
* Other categorical variables in the meta data to include in model, such as batch or Patient  

**reduction**  
* "UMAP" - re-embed the UMAP graph  
* "TSNE" - re-embed the TSNE graph  

**dim**  
* The number of dimensions of the correct PCA to use, i.e., 1:20 

```{r}
#To save space, the example Seurat only includes the integrated RNA values.
#In order to regress out the effects of the clonotype, we need the RNA values
#For the purposes of this demonstration, we are just going to assign the RNA
#assay as the integrated assay, please don't to this for your data
screp_example[["RNA"]] <- screp_example@assays$integrated

#Quick Sampling just so it does not take too long
set.seed(123)
screp_example2 <- screp_example

screp_example2 <- regressClonotype(screp_example2, cloneCall = "gene+nt", 
              groupVariable = "Patient", reduction = "UMAP", dims = 1:10)

#Difference Between Old and New UMAP
DimPlot(screp_example, reduction = "umap")
DimPlot(screp_example, reduction = "umap")
```
