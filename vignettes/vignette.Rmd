...
suppressPackageStartupMessages(invisible(library(BiocManager)))
...
+ Change the **palette** parameter to desired color scheme. This approach uses the built-in palettes of grDevices and you can access the list of color schemes using ``hcl.pals()``.  
+ Add an additional ggplot layer with new color scheme. 
...
quantContig(combined, 
            ...
            chain = "both", 
            scale = TRUE) + 
  scale_color_manual(values = hcl.colors(8,"geyser"))
...
Another option here is to be able to define the visualization by data classes. Here we used the *combineTCR()* to define the **Type** variable as part of the naming structure. We can the **group.by** to specifically use a column in the data set to organize the visualization.
...
A visualizations of the relative usage of genes of the TCR or BCR, using `vizGenes()`. 
...
We can also use `vizGenes()` to look at the usage of genes in a single chain. So for example say we are interesting in the difference in TRB V and J usage between lung and peripheral blood samples - we can easily take a look at this using the following code:
...
We first want to look at the distribution of peripheral blood versus lung T cells. We can use the same color scheme as the rest of the scRepertoire package by calling the object **colorblind_vector** using the following hex codes. 
...
Seurat::DimPlot(scRep_example, group.by = "cloneType") +
    scale_color_manual(values=colorblind_vector[c(2,4,6)])
```
...
alluvialClonotypes(scRep_example, 
    ...
    scale_fill_manual(values = c("grey", colorblind_vector[7]))
...
This can also be used if we wanted to explore just the lung-specific T cells by just subsetting the single-cell object.
...
## 4. clonalOverlap  
```{r}
clonalOverlap(combined2, cloneCall="aa", method="cosine")
