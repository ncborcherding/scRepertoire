...
## Combining Contigs from other pipelines 

We are beginning to support contig sequences from other pipelines, the first being VDJ reconstruction from [TRUST4](https://github.com/liulab-dfci/TRUST4). We have added `combineTRUST4()` that will parse the contig output from TRUST4 into a format usable by the rest of the scRepertoire package. This function includes parsing T-AB, T-GD and B cells sequences and uses the same parameters as described above. 
...
```{r}
quantContig(combined, cloneCall="gene", group.by = "ID", scale = TRUE)
```
...
abundanceContig(combined, cloneCall = "gene", group.by = "ID", scale = FALSE)
```
...
```{r}
abundanceContig(combined, group.by = "ID", scale = TRUE)
```
...
lengthContig(combined, cloneCall="aa", chain = "TRA") 
```
...
the defualt is to sperate by the list element names.
...
We can also use `vizGenes()` to look at the usage of genes in a single chain. So for example say we are interesting in the diference in TRB V and J usage between tumor and peripheral blood samples - we can easily take a look at this using the following code:

```{r}
#Peripheral Blood
vizGenes(combined[c(1,3,5)], gene = "V", chain = "TRB", 
         y.axis = "J", plot = "heatmap", scale = TRUE, order = "variance")

#Tumor Infiltrating
vizGenes(combined[c(2,4,6)], gene = "V", chain = "TRB", 
         y.axis = "J", plot = "heatmap", scale = TRUE, order = "variance")
```
...
seurat <- combineExpression(list.receptors, seurat, 
...
clonalOverlay(screp_example, reduction = "umap", 
              freq.cutpoint = 1, bins = 10, facet = "Patient") + 
                guides(color = "none")
```
...
**filter.identity**  
* For the identity chosen to visualize, show the to and from netwrok connections for a single group  
...
circles <- getCirclize(screp_example, group.by = "cluster")
...
circles <- getCirclize(subset, group.by = "cluster")
...
### Single-cell object support (Dev Version Only)
...
```{r}
seurat <- clusterTCR(screp_example,
                     chain = "TRB",
                     group.by = "Patient", 
                     sequence = "aa", 
                     threshold = 0.85)

DimPlot(screp_example, group.by = "TRB_cluster") + 
  NoLegend()
```
...
A new metric proposed by [Massimo et al](https://www.biorxiv.org/content/10.1101/2021.09.20.458613v1), clonal bias, like Startac is a clonal metric that seeks to quantify how individual clones are skewed towards a  specific cellular compartment or cluster. With the minimal example present in the in-built vignette, this will not be as enlightening as when we use a full single-cell object.
...
```{r}
combined2 <- expression2List(screp_example, split.by = "cluster")
combined3 <- expression2List(sce, split.by = "cluster")
```
