...
pc <- prcomp(df.genes, scale. = TRUE) 

# Getting data frame to plot from
df_plot <- as.data.frame(cbind(pc$x[,1:2], rownames(df.genes)))
...
pc.vj <- prcomp(df.vj, scale. = TRUE) # Scale the data for PCA

# Getting data frame to plot from
df_plot_vj <- as.data.frame(cbind(pc.vj$x[,1:2], rownames(df.vj)))
