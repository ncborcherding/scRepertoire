...
exported_clones <- exportClones(scRep_example,
                                write.file = FALSE,
                                format = "tcrpheno") 
exported_cloness <- na.omit(exported_clones)
...
```{r}
p1 <- FeaturePlot(scRep_example, 
                  features = "TCRmem", 
                  order = TRUE) +
        scale_color_viridis(option = "B") 

p2 <- FeaturePlot(scRep_example, 
                  features = "TCRreg", 
                  order = TRUE) +
        scale_color_viridis(option = "B") 

p3 <- FeaturePlot(scRep_example, 
                  features = "TCR.8", 
                  order = TRUE) +
        scale_color_viridis(option = "B") 

p4 <- FeaturePlot(scRep_example, 
                  features = "TCRinnate", 
                  order = TRUE) +
        scale_color_viridis(option = "B") 
...
```{r}
FeaturePlot(scRep_example, 
            features = c("CD4","CD8A", "FOXP3", "KLRB1"), 
            cols = viridis_pal(option = "B")(50))
```
