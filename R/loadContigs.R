...
#'
#' @description
#' This function loads and processes contig data from various single-cell immune receptor sequencing formats.
#' It reads data from a directory (recursively) or from an already loaded list/data frame, transforms it to a common structure,
#' and returns a list of contigs ready for downstream analysis with [combineTCR()] or [combineBCR()].
#'
...
#' - **Immcantation**: `"data.tsv"` (or similar)
#' - **JSON**: `".json"`
...
#'
#' @param input A directory path containing contig files or a list/data frame of pre-loaded contig data.
#' @param format A string specifying the data format. Must be one of:
#' "10X", "AIRR", "BD", "Dandelion", "JSON", "MiXCR", "ParseBio", "Omniscope", "TRUST4", "WAT3R", or "Immcantation".
#'
#' @return A list of contigs formatted for use with [combineTCR()] or [combineBCR()]. Rows containing only NA values
#' (aside from the barcode) are dropped.
#'
#' @examples
#' TRUST4 <- read.csv("https://www.borch.dev/uploads/contigs/TRUST4_contigs.csv")
#' contig_list <- loadContigs(TRUST4, format = "TRUST4")
#'
#' BD <- read.csv("https://www.borch.dev/uploads/contigs/BD_contigs.csv")
#' contig_list <- loadContigs(BD, format = "BD")
#'
#' WAT3R <- read.csv("https://www.borch.dev/uploads/contigs/WAT3R_contigs.csv")
#' contig_list <- loadContigs(WAT3R, format = "WAT3R")
#'
#' @importFrom utils read.csv read.delim
#' @importFrom rjson fromJSON
#' @export
loadContigs <- function(input, format = "10X") {
  # Validate input type and format string
  assert_that(
    is.string(input) || is.list(input) || is.data.frame(input),
    is.string(format),
    isIn(format, c("10X", "AIRR", "BD", "Dandelion", "JSON", "MiXCR",
                   "ParseBio", "TRUST4", "WAT3R", "Immcantation"))
  )
  
  # If input is a directory path, load files recursively
  rawDataDfList <- if (is.character(input)) {
    ...
    file_patterns <- format_patterns[[format]]
    # Build regex to match any file ending with one of the file patterns
    pattern_regex <- paste0(".*(", paste(file_patterns, collapse = "|"), ")$")
    ...
    if (length(contig_files) == 0) {
      warning("No contig files found in the specified directory.")
      return(list())
    }
    ...
    lapply(contig_files, reader)
  } else {
    .checkList(input)
  }
  
  # Select the appropriate parser function based on format
  ...
  # Process and clean contig data, dropping rows that are entirely NA (except barcode)
...
.parseTRUST4 <- function(df_list) {
  ...
  # Parse chain information from the v_gene column
...
.parseImmcantation <- function(df_list) {
  lapply(df_list, function(df) {
    ...
    # Extract the barcode (portion before any underscore)
...
.parseParse <- function(df_list) {
  lapply(df_list, function(df) {
    ...
    if (isTcell) {
      ...
      # Process B-cell chains: IGH, IGK, and IGL
    ...
    # Remove rows where columns 2 through 8 are all NA
