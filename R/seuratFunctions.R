...
#'
#' This function adds the immune receptor information to the seurat or SCE object 
#' to the meta data. By defualt this function also calculates the frequencies of 
#' the clonotypes by sequencing run (groupBy = "none"). To change how the 
#' frequencies are calculated, select a column header for the groupBy variable. 
#' Importantly, before using combineExpression() ensure the barcodes of the 
#' seurat or SCE object match the barcodes in the output of the combinedContig() 
#' call. Check changeNames() to change the prefix of the seurat object. If the 
#' dominant clonotypes have a greater frequency than 500, adjust the cloneTypes variable.
#'
#' @examples
#' \donttest{
...
#' or CDR3 amino acid (aa), or
#' CDR3 gene+nucleotide (gene+nt).
#' @param groupBy The column label in the combined contig object in which clonotype 
...
combineExpression <- function(df,
    ...
    if (length(df) == 0 | length(sc) == 0) {
        stop("Make sure you are adding the combined contigs and Seurat/SCE object 
                to the combineExpression() function")
    }
...
#        data2<- data2[ , -which(names(data2) %in% c(groupBy))]
    ...
    for (i in 2:length(cloneTypes)) {
        Con.df$cloneType <- ifelse(Con.df$Frequency > cloneTypes[i-1] & Con.df$Frequency <= cloneTypes[i], 
                                names(cloneTypes[i]), Con.df$cloneType)
    }
...
highlightClonotypes <- function(sc, cloneCall = c("gene", "nt", "aa", "gene+nt"), sequence = NULL){
    if (!inherits(x=sc, what ="Seurat")) {
...
#' ggalluvial package, which requires the aesthetics to be part of the axes that are
#' visualized. Therefore, alpha, facet, and color should be part of the the axes you 
#' wish to view or will add an additional stratum/column to the end of the graph.
#'
#' @examples
#' \donttest{
...
#' For SCE objects, the cluster variable must be in the meta data under "cluster".
#' @param cloneCall How to call the clonotype - CDR3 gene (gene), CDR3 nucleotide (nt) 
...
alluvialClonotypes <- function(sc, cloneCall = c("gene", "nt", "aa", "gene+nt"), 
                                y.axes = NULL, color = NULL, alpha = NULL, facet = NULL) {

    if (!inherits(x=sc, what ="Seurat") | inherits(x=sc, what ="SummarizedExperiment")){
        stop("Object indicated is not of class 'Seurat' or 'SummarizedExperiment', make sure you 
...
StartracDiversity <- function(sc, type = "Type", sample = NULL, by = c("overall")) {
    if (inherits(x=sc, what ="Seurat")) {
    ...
    if (is.null(sample)) {
        ...
        processed <- data.frame(rownames(meta), meta$CTstrict, meta$clone.status, meta[,sample], 
                                meta[,"majorCluster"], meta[,type], stringsAsFactors = FALSE)
